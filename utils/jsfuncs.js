...
class JSFunctionExtractor {
    ...
    extract() {
        try {
            ...
            console.error(`Warning: Could not parse ${this.relativePath}: ${error.message}`);
        }
    ...
    extractWithTypeScript() {
        // Implementation would require @typescript-eslint/parser
    ...
    processMatch(match) {
        ...
        if (fullMatch.includes('function')) {
            ...
            const functionMatch = fullMatch.match(/function\s*\*?\s+(\w+)\s*\(([^)]*)\)(?:\s*:\s*([^{]+?))?/);
            if (functionMatch) {
            ...
            // Arrow function
            const arrowMatch = fullMatch.match(/(?:const|let|var)\s+(\w+)\s*=\s*(?:async\s+)?\(([^)]*)\)(?:\s*:\s*([^=]+?))?\s*=>/);
            if (arrowMatch) {
                name = arrowMatch[1];
            ...
            const methodMatch = fullMatch.match(/(?:(?:public|private|protected|static)\s+)?(?:async\s+)?(?:get\s+|set\s+)?(\w+)\s*\(([^)]*)\)(?:\s*:\s*([^{]+?))?/);
            if (methodMatch) {
                name = methodMatch[1];
        ...
        if (name) {
            const funcInfo = new FunctionInfo({
                ...
                exported,
                name,
                className: this.classStack.length > 0 ? this.classStack[this.classStack.length - 1] : null,
                params,
                returns: returnType,
